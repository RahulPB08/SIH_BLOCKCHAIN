...
contract Queue is Script {
    ...
    function run() external {
        ...
        address boxV2Address = DevOpsTools.get_most_recent_deployment(
         ...
         console.log("New version:", BoxV1(proxyAddress).version());
        ...
        console.log("minDelay (secs):", minDelay);
        // On local development chain, advance time and blocks
        if (block.chainid == LOCAL_CHAIN_ID) {
            vm.warp(block.timestamp + MIN_DELAY + 1);
            vm.roll(block.number + 1);
            console.log("Advanced time and block for local chain");
        }
        ...
        // If BoxV2 has a version function, you can call it to verify
